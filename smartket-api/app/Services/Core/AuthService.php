...
class AuthService
{
    ...
    public function getUserProfileData($user, $tenant = null): array
    {
        ...
        if (!$tenant && !($user instanceof \App\Models\Core\Staff)) {
        ...
        if ($user instanceof \App\Models\Core\Staff) {
            ...
            try {
                $userBranches = $tenant->execute(fn() => \App\Models\Core\Branch::where('is_active', true)->get(['id', 'name', 'is_active'])->toArray());
            } catch (\Exception $e) {
                \Log::warning("Error loading branches for owner in getUserProfileData: " . $e->getMessage());
                $userBranches = []; // Fallback - Sin sucursales
            }
        }
